...
class PricingService:
    ...
    def __init__(self, region: Optional[str] = None):
        ...
        """
        self.logger = setup_logger("PricingService")
        ...
        self.pricing_client = boto3.client(
            'pricing', region_name=client_region)
        self.logger.debug(
            f"Initialized pricing client with region: {client_region}")
    ...
    def _get_model_name(self, model_id: str) -> str:
        ...
        if "arn:aws:bedrock" in model_id:
            ...
            if parts and ":" in parts[-1]:
                ...
                self.logger.debug(
                    f"Extracted model name from ARN: {model_name}")
        else:
            model_name = model_id
    ...
    def load_model_pricing(
        ...
        Returns:
            True if pricing data was successfully loaded, False otherwise
        """
        try:
            self.logger.debug(
                f"Fetching pricing data for model: {model_id} in region {region}")
            ...
            try:
                ...
                if response.get('PriceList') and len(response['PriceList']) > 0:
                    ...
                    self.logger.debug(
                        f"Found {len(response['PriceList'])} pricing items to parse")
                    ...
                    for price_item in response['PriceList']:
                        ...
                        usage_type = price_data.get('product', {}).get(
                            'attributes', {}).get('usagetype', '')
                        inference_type = price_data.get('product', {}).get(
                            'attributes', {}).get('inferenceType', '')
                        feature = price_data.get('product', {}).get(
                        ...
                        price_dimensions = on_demand[on_demand_key].get(
                            'priceDimensions', {})
                        ...
                        if (not price_dimension or
                            ...
                            self.logger.debug(
                                "Invalid price dimension structure")
                            continue
                        ...
                        if (('Input' in inference_type or 'input' in usage_type) and
                            ...
                            self.logger.debug(
                                f"Found input token price: ${price_per_unit}/1K tokens ({usage_type})")
                            ...
                            self.logger.debug(
                                f"Found output token price: ${price_per_unit}/1K tokens ({usage_type})")
                    ...
                    if found_prompt_price and found_completion_price:
                        ...
                        self.logger.warning(
                            f"No applicable pricing data found for model {model_id}, using defaults")
                        return False
                else:
                    self.logger.warning(
                        f"No pricing data found for model {model_id}, using defaults")
                    return False

            except ClientError as e:
                self.logger.warning(
                    f"Error fetching pricing data: {e}. Using default pricing.")

            return False

        except Exception as e:
            self.logger.warning(
                f"Failed to load model pricing: {e}. Using default pricing.")
            return False
    ...
    def calculate_cost(self, prompt_tokens: int, completion_tokens: int) -> Dict[str, float]:
        ...
        prompt_cost = prompt_tokens * \
            (self.current_pricing['prompt_rate'] / 1000)
        completion_cost = completion_tokens * \
            (self.current_pricing['completion_rate'] / 1000)
