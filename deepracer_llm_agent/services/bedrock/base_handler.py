...
class ModelHandler(ABC):
    ...
    def invoke_model(self, request_body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.debug(f"Invoking {self.model_id} with request: {request_json[:200]}...")
    ...
    def extract_driving_action(self, response_text: str) -> Dict[str, Any]:
        ...
        """
        raise NotImplementedError("Subclasses must implement extract_driving_action()")
    
    @abstractmethod
    def process(self, prompt: str, image_data: Optional[str] = None) -> Dict[str, Any]:
        """
        Process a prompt with image and return a driving action. Override in subclasses.
